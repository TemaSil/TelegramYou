        ...
        LazyColumn(
            ...
            item {
                OutlinedTextField(
                    ...
                    // Done on the keyboard creates it, for a group: the name
                    // is the last thing typed, and reaching up past the
                    // keyboard for the button is a stretch the key saves. A
                    // channel's next field is its description, so there it
                    // moves on instead.
                    keyboardOptions = KeyboardOptions(
                        imeAction = if (kind == NewChatKind.Group) ImeAction.Done else ImeAction.Next
                    ),
                    keyboardActions = KeyboardActions(onDone = { if (state.canCreate) onCreate() }),
                    modifier = Modifier
                        .fillMaxWidth()
